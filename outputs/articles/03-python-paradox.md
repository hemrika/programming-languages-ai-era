...
Python is not the language I expected to land at #5. Walking into the framework, I assumed the language with the deepest AI-systems ecosystem in the cohort — the one every model-provider ships first-party SDKs for, the one that owns the data-science pipeline, the one running in every notebook on every research desk — would settle near the top. The matrix put it at 3.71, fifth in the cohort, tied with Rust, and 0.30 points behind the joint leaders TypeScript and Go (4.01) and 0.28 behind .NET (3.99). The reason is not that the ecosystem story is wrong. The ecosystem story is exactly right: Python hits the cohort ceiling on AI-Systems Ecosystem — what the third-party ecosystem provides for AI integration: community libraries and commercial SDKs that aren't shipped by the language steward — at AIE=5.0, and on Structured-Output Ecosystem — what third-party libraries provide for parsing LLM output (Pydantic, Instructor, Outlines, and the like) — at SOE=5.0, matched only by TypeScript; no other language reaches either ceiling. The score is held down by a different half of the row — by what Python's steward does not ship, by what its type system does not enforce, and by who maintains the libraries the AI half depends on.
...
Read the row two ways. Read AIE and SOE alone and Python sits at the cohort ceiling on both, tied with TypeScript and unmatched by anyone else. Read AI-Systems Native — what the language steward ships first-party for AI integration: LLM SDKs, agent frameworks, MCP servers, inference runtimes shipped as part of the language platform — at AIN=1.5 (tied at the cohort floor with TypeScript and Rust), Structured-Output Native — what the language and standard library provide for schema-validated parsing of LLM output: type-safe deserialization, sum types, exhaustive pattern matching — at SON=2.5 (fourth-from-bottom; only Rust at 2.0, Elixir at 1.5, and C++ alone at 1.0 sit lower), and Ecosystem Dependency Risk — the framework's measure of how resilient the maintainership of load-bearing libraries is, where higher means lower risk and reflects how concentrated or distributed the maintainership of critical libraries is — at EDR=2.5 (alone at the cohort floor) and Python sits with the bottom of the matrix. The weighted total averages the two halves into 3.71, tied with Rust at #5/#6, behind a four-language top tier (TypeScript and Go at 4.01, .NET at 3.99, Kotlin at 3.85) compressed inside 0.16 of weighted score.
