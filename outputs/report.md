...
*A greenfield evaluation of 14 languages against the demands of AI-assisted and AI-agentic software development.*

## TL;DR

Across 14 languages, scored on five weighted dimensions and read through four cross-cutting lenses, **Go (4.45)**, **TypeScript (4.25)**, **Rust (4.15)**, **Kotlin (4.00)** and **.NET / C# (4.00)** form the top tier for greenfield AI-era projects. Python (3.95) sits just below, retained by ML-ecosystem velocity but dropped from the top tier once installed-base credit is removed. Swift, Java, Zig, Elixir and Haskell occupy a middle tier where each has a real structural strength offset by a load-bearing operability or ecosystem weakness. Julia, Mojo and C++ trail at the bottom — Julia for narrow scope, Mojo for ecosystem stage, C++ for memory-safety pressure that has crossed from technical preference into regulatory selection criterion.
...
The original dynamic-versus-static binary is becoming a spectrum. TypeScript's static layer over JavaScript [typescript-001] with discriminated unions [typescript-002] and structural typing [typescript-003] reaches an AI-era profile competitive with mature static languages — score 4.25, ahead of Kotlin and .NET. Python's typing system has advanced through PEPs 484, 526, 544, 612, 646, 692, 695 [python-016], with PEP 695 generic syntax [python-004] and pyright/Pylance bundled stubs [python-018] giving agents enough signal to operate in well-typed Python codebases. Elixir is researching a set-theoretic type system [elixir-007, elixir-013] and already supports Dialyzer's success-typing analysis [elixir-006]. The gap that remains is the bridge between languages that have invested in static structure and languages that have not: Julia's optional annotations [julia-007] feed inference and dispatch [julia-013] but type-stability discipline is hand-maintained [julia-016]; plain JavaScript continues to lose ground to TypeScript. The headline is that *gradual typing* — not the original static or dynamic origin — is the most predictive feature. The full reading lives in `comparisons/dynamic-vs-static.md`.
...
Strongest forward case: explicit design — no hidden control flow, allocations, or operator overloading [zig-001], explicit allocator passing [zig-002], unified comptime [zig-003], and first-class cross-compilation [zig-005, zig-013]. **Load-bearing risk:** pre-1.0 status with continuing language and stdlib churn [zig-006, zig-010], and no compile-time memory-safety guarantees comparable to Rust's [zig-009].
...
Strongest forward case: Hindley-Milner inference over a pure non-strict core [haskell-001, haskell-002] with Liquid Haskell extending the surface to refinement types and compile-time logical specifications [haskell-005]. **Load-bearing risk:** GHC's large optional-extensions surface complicates a single canonical dialect [haskell-004], lazy-by-default evaluation creates space-leak risk [haskell-015], and typeclass and higher-kinded type machinery remain unfamiliar to mainstream developers [haskell-012].

### Julia — 2.75 — *Defensible in numerical computing, narrow elsewhere.*

Strongest forward case: multiple dispatch combined with type inference enables generic library composition with high-performance specialized code [julia-002, julia-003], with native code caching closing time-to-first-execution [julia-004] and a credible accelerator ecosystem [julia-014]. **Load-bearing risk:** central numerical-computing focus narrows general-purpose future fit [julia-012] and type-stability discipline must be hand-maintained [julia-016].

### Mojo — 2.65 — *A forward bet on AI-native compute kernels with active risk management required.*

Strongest forward case: MLIR-based compilation targeting heterogeneous hardware [mojo-002, mojo-013], compile-time parameters with ownership conventions [mojo-004, mojo-005, mojo-014], and frequent releases under public changelog [mojo-015]. **Load-bearing risk:** single-vendor governance under Modular [mojo-008], pre-1.0 status [mojo-010], and small AI-training-corpus footprint [mojo-012].
...
| Systems / infrastructure / security-sensitive | Rust | Zig, Go | Verification and safety [rust-001, rust-015], accepting compile-time and learning-curve cost [rust-020, rust-025]. |
| Data / AI/ML / scripting | Python | TypeScript (for data products), Julia (numerics) | ML ecosystem velocity [python-012] over verification gap [python-002, python-019]. Treat type discipline as day-one investment. |
...
| AI-native compute kernels | Mojo (forward bet), Rust | C++ (only where unavoidable) | Hardware-portable performance [mojo-002, mojo-013], accepting ecosystem narrowness [mojo-008, mojo-012]. |
...
The snapshot date is **2026-04-30**. Several languages are evolving rapidly and the scores are time-bound: Mojo's pre-1.0 trajectory [mojo-010] and frequent release cadence [mojo-015] mean ecosystem-stage scores could shift materially within twelve months; Zig's pre-1.0 churn [zig-006] is similar; Julia's accelerator and parser improvements [julia-005, julia-014] are continuing to land. Elixir's set-theoretic type system [elixir-007, elixir-013] could move that language's verification score before its next major release.
...
- **Claims** — `claims/<language>.yaml`, 255 atomic claims across 14 languages, each with a primary-source citation and explicit polarity.
- **Sources** — `sources/<language>-sources.yaml`, 181 source entries with publication metadata.
