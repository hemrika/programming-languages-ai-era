...
The headline is that v0.4 — splitting AI-systems and structured-output into native + ecosystem halves and adding an ecosystem-dependency-risk dimension — reshuffles the ranking sharply. **Go takes the top slot** because its operability and human-cognition advantages carry through unchanged while its commercial-first-party SDK ecosystem (Anthropic Go SDK, OpenAI Go SDK) and Google language stewardship anchor the dependency-risk profile. **Python falls from #2 in v0.3 (4.25) to #6 in v0.4 (3.74)** — the largest movement in the cohort — because v0.3 implicitly credited Pydantic, Instructor, Outlines, and the LangChain Python ecosystem to Python's full-weighted AI dimension; v0.4 splits this into AIN (native = essentially absent because PSF ships no AI packages) and AIE (5.0, best in cohort), and introduces an EDR axis on which Python scores 2.5 because Pydantic is community-multi-maintainer, Instructor is single-maintainer, Outlines is small-commercial. **.NET climbs to third** because Microsoft's first-party Semantic Kernel + Microsoft.Extensions.AI + ONNX Runtime .NET stack is now correctly credited at AIN=4.0 and the EDR=4.5 score (highest in cohort) reflects Microsoft-vendor anchoring.
...
- **Runtime and ecosystem (10%)** — can it run production systems? *(reduced from 15% in v0.4 to fund EDR.)*
...
### 1. Go takes the top slot under v0.4.

v0.3 ranked TypeScript #1 (4.38) on strict-mode AO=5 plus Zod-driven SO=5.0 and AI=4.5. v0.4 keeps operability at 20% but splits the AI and SO dimensions into native + ecosystem halves and adds the dependency-risk axis. Go's HC=5 / AO=5 lead carries through unchanged at full weight, while TypeScript's AIE=5 / SOE=5 lifts dilute against AIN=1.5 / SON=2.5. The 0.03-point gap between Go (4.04) and TypeScript (4.01) reads as a near-tie at the top, not a clear winner. The 7th-dimension Python/Go swap from v0.3 reverses: Go now sits 0.30 above Python (3.74).

### 2. Python's structural exposure is now visible.

Under v0.3, Python's 4.25 was lifted by AI=5 + SO=5 at a combined 25% weight. v0.4 splits both into native + ecosystem and adds the dependency-risk axis. Python earns AIE=5.0 [python-021, python-022, python-023, python-024, python-025] and SOE=5.0 [python-026, python-027, python-028, python-029] — still tier-leading on the ecosystem halves. But AIN=1.5 (PSF ships no AI packages [python-036]), SON=2.5 (PEP 484 hints are advisory at runtime [python-030]), and EDR=2.5 (Pydantic community-multi-maintainer [python-031], Instructor single-maintainer [python-032], Outlines small-commercial [python-033]) reflect the dependency profile that v0.3 absorbed implicitly. The net effect is a 0.51-point drop from #2 to #6.
...
### TypeScript — 4.01 — *Default for application work in the JavaScript-shaped world; near-tie with Go under v0.4.*
...
### .NET (C#) — 3.96 — *Climbs to third on the strength of the v0.4 native-AI split.*
...
### Kotlin — 3.85 — *Default for JVM application work; rises to fourth under v0.4 EDR credit.*
