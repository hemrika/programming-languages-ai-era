...
In v0.3, **structured-output maturity** is now scored as its own dimension (10% weight) rather than absorbed into AI-systems interoperability or machine cognition. The schema-validation, LLM tool-call typing, constrained-generation, and JSON-Schema-from-types capabilities — Pydantic, Zod, Serde+schemars, Jackson + Bean Validation, kotlinx-serialization, System.Text.Json + JsonSchemaExporter, Codable, Ecto changesets, Outlines, llama.cpp GBNF, Vercel AI SDK structured outputs, Instructor, Semantic Kernel function calling — are concentrated enough to warrant separate evaluation and differentiate languages whose general AI-systems integration looks similar at the top level. The dimension does not warrant a fifth lens; it functions as a finer-grained capability axis within the four-lens reading.
