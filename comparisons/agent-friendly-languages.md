...
- Default to **Go** for backend services where its structural weaknesses (no generics until recently, limited abstraction) are acceptable.
- Default to **TypeScript** for application work where its ecosystem fits.
- Choose **Rust** only when verification or safety pressure outweighs the operability cost.
